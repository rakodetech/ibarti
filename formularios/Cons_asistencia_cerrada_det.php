...
function FiltarRol(ValorN){
	...
	var Nmenu = document.getElementById("Nmenu").value;
	var mod   = document.getElementById("mod").value;
	var href  = "inicio.php?area=formularios/Cons_asistencia_cerrada&Nmenu="+Nmenu+"&mod="+mod+"&co_cont="+contracto+"&rol="+ValorN+"";
	window.location.href=""+href+"";
//	alert(href);
}
...
$vinculo= "formularios/Cons_asistencia_cerrada_det&Nmenu=$Nmenu&mod=".$_GET['mod']."";

if (!isset($co_cont) || !isset($cod_rol) || !isset($cod_apertura)  ){
exit();
}

$bd = new DataBase();
	$sql04 = "SELECT asistencia_apertura.fec_diaria, Dia_semana(asistencia_apertura.fec_diaria)  AS dia,
	                  asistencia_apertura.fec_cierre,
                     contractos.descripcion AS contracto, roles.descripcion AS rol
                FROM asistencia_apertura , contractos , roles
...
<hr />
	<table width="98%">
		<tr><td width="12%" class="etiqueta"><?php echo $leng['rol']?>: </td>
			<td width="40%"><select id="roles" style="width:200px;" onchange="FiltarRol(this.value)">
...
<hr /><div id="listar"><form id="asistencia_01" name="asistencia_01" action="scripts/sc_asistencia.php"
		...
		</tr><?php	echo '<tr class="fondo01"><td><select name="trabajador" id="trabajador" style="width:210px;">
		...
		$sql_ubicacion = "SELECT usuario_clientes.cod_ubicacion, clientes_ubicacion.descripcion AS ubicacion
					...
					echo'</select></td>
           	      <td id="conceptoX'.$i.'"><select id="concepto'.$i.'" style="width:75px;" onchange="spryValidarSelect(this.id)">								   <option value="'.$datos["cod_concepto"].'">'.$datos["abrev"].Feriado_as($datos["feriado"], "FER").Feriado_as($datos["NL"], "NL").'</option>';
						   ...
						   echo '<option value="'.$row04[0].'">'.$row04[2].Feriado_as($datos["feriado"], "FER").Feriado_as($datos["NL"], "NL").'</option>';}echo'</select><br/><input type="hidden" style="width:0px" id="feriado'.$i.'" name="feriado'.$i.'" value="'.$datos["feriado"].'" /><input
		                          type="hidden" style="width:0px"  id="NL'.$i.'" name="NL'.$i.'" value="'.$datos["NL"].'" /></td>
	   		    <td><input type="text" id="horaD'.$i.'" style="width:45px" value="'.$datos["hora_extra"].'" maxlength="3"
