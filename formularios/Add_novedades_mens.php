...
if($metodo == 'modificar'){
   $titulo = "MODIFICAR $titulo";
	...
	$sql = " SELECT nov_mensajeria.cod_novedad, novedades.descripcion AS novedad,
                    ficha.cod_ficha, ficha.cedula, CONCAT(ficha.apellidos, ' ', ficha.nombres) AS trabajador,
                    nov_mensajeria.descripcion
               FROM nov_mensajeria , novedades , ficha
              WHERE nov_mensajeria.codigo = $cod_mensajeria 
			    AND nov_mensajeria.cod_ficha = ficha.cod_ficha 
	...
	$trabajador    = $result['cod_ficha'].' - '.' ('.$result['cedula'].') '.$result['trabajador'];
    $novedad       = $result['novedad'];
    $observacion   = $result['descripcion'];
	$metodo = "agregar";
    ...
    $cod_status = $row02[0];
    $status     = $row02[1];
	}elseif($metodo == "mensaje"){  // NOVEDAD MENSAJERIA
	$sql   = "SELECT nov_mensajeria.fecha,  nov_mensajeria.telefono, 
                  	 nov_mensajeria.cod_ficha, ficha.cedula, 
	                 nov_clasif.descripcion AS nov_clasif, novedades.descripcion AS novedades, 
				     CONCAT(ficha.apellidos, ' ',ficha.nombres) AS trabajador, 
                     nov_mensajeria.descripcion,  Valores(nov_mensajeria.`status`) AS status,
...
?><form action="scripts/sc_<?php echo $archivo;?>.php" method="post" name="add" id="add"> 
   ...
   <tr>
      <td class="etiqueta">Novedad:</td>
      	<td id="select01"><select name="novedad" style="width:200px">
							<option value="<?php echo $cod_novedad;?>"><?php echo $novedad;?></option>
          <?php  	$sql   = " SELECT novedades.codigo, novedades.descripcion AS novedad
			                     FROM novedades , control
                                WHERE novedades.cod_nov_clasif = control.cod_nov_clasif_sms
    ...
    <tr>
      ...
      <td class="etiqueta">Ubicaci&oacute;n:</td>
      	<td id="select03"><select name="ubicacion" style="width:200px">
   ...
   <td colspan="2">&nbsp;</td>
    ...
    <tr>
      ...
      <td id="textarea01"><textarea  name="observacion" cols="45" rows="3"><?php echo $observacion;?></textarea>
      ...
      <td id="textarea02"><textarea  name="repuesta" cols="45" rows="3" readonly="readonly"><?php echo $respuesta;?></textarea>
...
<span class="art-button-wrapper">
	        ...
	        <input name="href" type="hidden" value="<?php echo $href;?>"/>	
   	        <input name="cod_mensajeria" type="hidden" value="<?php echo $cod_mensajeria;?>"/>
            </div>
</form>
...
<form action="scripts/sc_<?php echo $archivo;?>.php" method="post" name="add" id="add"> 
   ...
   <tr>
    <td class="etiqueta" width="15%">Codigo:</td>
    <td width="35%"><input type="text" size="30" name="codigo" value="<?php echo$cod_mensajeria;?>" readonly="readonly"/></td>
      <td class="etiqueta" width="15%">Fecha:</td>
      <td width="35%"><input type="text" size="30" name="codigo" value="<?php echo $row02['fecha'];?>" readonly="readonly"/></td>
     </tr>	    
    <tr>
    <td class="etiqueta">Ficha:</td>
    <td><input type="text" size="30" name="codigo" value="<?php echo $row02['cod_ficha'], '('.$row02['cedula'].')';?>" readonly="readonly"/></td>
      <td class="etiqueta">Trabajador:</td>
      <td><input type="text" size="50" name="codigo" value="<?php echo $row02['trabajador'];?>" readonly="readonly"/></td>
    </tr>
    <tr>
    <td class="etiqueta">Novedad Clasificacion:</td>
    <td><input type="text" size="40" name="codigo" value="<?php echo $row02['nov_clasif'];?>" readonly="readonly"/></td>
      <td class="etiqueta">Novedad:</td>
      <td><input type="text" size="40" name="codigo" value="<?php echo $row02['novedades'];?>" readonly="readonly"/></td>
    </tr>    
    <tr>
    <td class="etiqueta">Problema:</td>
    <td><textarea  name="direccion" cols="50" rows="3" readonly="readonly"><?php echo $row02['descripcion'];?></textarea></td>
      <td class="etiqueta">Solucion:</td>
	  <td><textarea  name="direccion" cols="50" rows="3" readonly="readonly"><?php echo $row02['descripcion'];?></textarea></td>
    </tr>    

    <tr>
    <td class="etiqueta">Status:</td>
    <td><input type="text" size="30" name="codigo" value="<?php echo $row02['status'];?>" readonly="readonly"/></td>
     <td colspan="2"></td> 
    </tr>    

    
    <tr>
	<td class="etiqueta">Ultimo us. Modifico:</td>
      <td><input type="text" size="50" name="codigo" value="<?php echo $row02['usuario_mod'];?>" readonly="readonly"/></td>
       <td class="etiqueta">Fecha Ultima Mod.:</td>
      <td><input type="text" size="30" readonly="readonly" value="<?php echo $fec_us_mod;?>"></td> 
	 </tr>
</table>
<div align="center">
             <span class="art-button-wrapper">
