...
{
	//Fatal error
	die('<b> <h1>FPDF Error:</h1></b> <h3>'.$msg.'</h3>');
}
...
function Output($name='', $dest='')
{
	...
	switch($dest)
	{
		case 'I':
			...
			if(ob_get_length())
				$this->Error('I01, Algunos Datos De Salidad No Pueden Ser Enviado Al  Archivo PDF');
			if(php_sapi_name()!='cli')
			{
				//We send to a browser
				header('Content-Type: application/pdf');
				if(headers_sent())
					//$this->Error('D01, Some data has already been output, can\'t send PDF file');
					$this->Error('I02, No hay datos para el criterio que selecciono.');
		...
		case 'D':
			...
			if(ob_get_length())
				//$this->Error('D01, Some data has already been output, can\'t send PDF file');
				  $this->Error(' D01, Algunos Datos De Salidad No Pueden Ser Enviado Al  Archivo PDF');
			header('Content-Type: application/x-download');
			if(headers_sent())
				//$this->Error(' D02, Some data has already been output, can\'t send PDF file');
				$this->Error(' D02, Algunos Datos De Salidad No Pueden Ser Enviado Al  Archivo PDF');
			header('Content-Length: '.strlen($this->buffer));
