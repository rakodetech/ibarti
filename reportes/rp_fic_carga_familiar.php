...
$Nmenu   = 521;
$mod     =  $_GET['mod'];
$titulo  = " REPORTE CARGAR DE FAMILIAR ";
$archivo = "reportes/rp_fic_carga_familiar_det.php?Nmenu=$Nmenu&mod=$mod";
...
function Add_filtroX(){  // CARGAR  ARCHIVO DE AJAX CON UN PARAMETRO //
	...
	if(error == 0){
	  ...
	  var parametros = {
				...
				$.ajax({
						data:  parametros,
						url:   'ajax_rp/Add_fic_carga_familiar.php',
						type:  'post',
						beforeSend: function () {
...
<hr />
	<table width="100%" class="etiqueta"><tr><td width="12%"><?php echo $leng['rol']?>:</td>
        ...
        <td width="12%"><?php echo $leng['region']?>:</td>
		 <td width="20%"><select name="region" id="region" style="width:120px;">
		 ...
		 <td width="12"><?php echo $leng['contrato']?>: </td>
      ...
      <td width="4%" id="cont_img"><img class="imgLink" src="imagenes/actualizar.png" border="0" onclick="Add_filtroX()"></td>
        ...
        <tr>
		 ...
		 <td>Parentesco: </td>
			<td><select name="parentesco" id="parentesco" style="width:120px;">
					<option value="TODOS">TODOS</option>
					<?php
	   			$query01 = $bd->consultar($sql_parentesco);
		 		while($row01=$bd->obtener_fila($query01,0)){
					 echo '<option value="'.$row01[0].'">'.$row01[1].'</option>';
			   }?></select></td>
...
<td><?php echo $leng['trabajador']?>:</td>
      <td colspan="5"><input  id="stdName" type="text" style="width:250px" disabled="disabled" />
