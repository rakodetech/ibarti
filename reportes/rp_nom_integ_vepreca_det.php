...
if(isset($reporte)){
    ...
    echo "</tbody>
		...
		if($reporte == 'txt'){
		 ...
		 $sql = " SELECT asistencia_apertura.fec_cierre, ficha.cod_ficha,
		                 SUBSTRING(ficha.cod_ficha,1, CHAR_LENGTH(ficha.cod_ficha)-3) AS cedula,                                                                            zona_asistencia(ficha.cod_ficha, \"$fecha_H\") AS zona,
						 CONCAT('0000',SUBSTRING(ficha.cod_ficha, CHAR_LENGTH(ficha.cod_ficha)-1, 2)) AS n_contrato,
	...
	//	Header("Content-Disposition: attachment; filename=../txt/asistencia.txt");
		Header("Content-Disposition: attachment; filename=".$archivo.".txt");
			while ($row01 = $bd->obtener_num($query01)){
			echo "".$row01[0].";".$row01[1].";".$row01[2].";".$row01[3].";".$row01[4].";".$row01[5].";".$row01[6].";\r\n";
			}
		}
	/*
