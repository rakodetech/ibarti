...
$Nmenu   = 5400;
$mod     =  $_GET['mod'];
$titulo  = " REPORTE ASISTENCIA PERFECTA ";
$archivo = "reportes/rp_nom_as_perfecta_det.php?Nmenu=$Nmenu&mod=$mod";
...
function Add_filtroX(){  // CARGAR  ARCHIVO DE AJAX CON UN PARAMETRO //
    ...
    var errorMessage = ' Campos De Fecha Incorrectas ';
	 ...
	 if( (rango == "") ||(rango < 1)){
    var errorMessage = errorMessage + ' \n Rango Debe Ser Mayor Un Numero ';
	 var error = error+1;
	}
	...
	if(error == 0){
		...
		ajax=nuevoAjax();
			ajax.open("POST", "ajax_rp/Add_nom_as_perfecta.php", true);
			...
			ajax.send("rango="+rango+"&rol="+rol+"&region="+region+"&estado="+estado+"&ciudad="+ciudad+"&contrato="+contrato+"&trabajador="+trabajador+"&fecha_desde="+fecha_desde+"&fecha_hasta="+fecha_hasta+"");
...
<form name="form_reportes" id="form_reportes" action="<?php echo $archivo;?>"  method="post" target="_blank">
	<hr /><table width="100%" class="etiqueta">
		...
		<td width="14%" id="integer01"><input type="text" name="rango" id="rango" maxlength="3" size="12" required/></td>
