...
if(isset($reporte)){
		...
		$where = " WHERE nov_check_list.fec_us_ing BETWEEN \"$fecha_D\" AND \"$fecha_H\"
					 ...
					 AND nov_check_list_det.cod_valor = nov_valores_det.cod_valores
                     AND nov_check_list_det.cod_valor = nov_valores.codigo
	...
	// QUERY A MOSTRAR //
          $sql = "SELECT nov_check_list.codigo, nov_check_list.fec_us_ing,
                         CONCAT(preingreso.apellidos, ' ', preingreso.nombres) AS superv,
                         clientes.nombre AS cliente, clientes_ubicacion.descripcion AS ubicacion,
						 nov_check_list.repuesta, nov_clasif.descripcion AS clasif,
                         nov_clasif.descripcion AS tipo, novedades.descripcion AS check_list ,
						 nov_valores.abrev, nov_valores_det.valor,  nov_valores.factor,
	 		     (SELECT Sum(nov_valores_det.valor) AS check_list_valor
                    FROM nov_check_list_det AS b , nov_valores_det
                   WHERE b.cod_check_list = nov_check_list.codigo
				     AND b.cod_novedades = nov_valores_det.cod_novedades
					 AND b.cod_valor = nov_valores_det.cod_valores
                GROUP BY b.cod_check_list) AS check_list_valor) AS check_list_valor,

                        (SELECT Sum(v_valores_max.valor_max) AS v_max
                                FROM v_valores_max WHERE v_valores_max.cod_cl_ubicacion = nov_check_list.cod_ubicacion
                                 AND v_valores_max.cod_nov_clasif = nov_check_list.cod_nov_clasif
                                 AND v_valores_max.cod_nov_tipo = nov_check_list.cod_nov_tipo) AS valor_max,

                        nov_check_list_det.observacion, nov_check_list.fec_us_mod, nov_status.descripcion AS nov_status
                   FROM nov_check_list , nov_check_list_det, novedades, nov_valores_det,
				        nov_clasif , nov_tipo, nov_valores, clientes ,
						clientes_ubicacion , ficha , preingreso , nov_status
                $where
              ORDER BY 3 ASC ";
	...
	if($reporte== 'excel'){
	 ...
	 echo "<tr><th>CODIGO </th><th>FECHA </th><th> SUPERVISOR </th><th> CLIENTE </th>
			   ...
			   <th> CHECK LIST </th><th> ABREVIATURA </th><th> VALOR </th> <th> FACTOR </th>
			   <th> VALOR CHECK LIST</th><th> VALOR MAX</th><th> OBSERVACION </th><th> FEC. ULTIMA MODIFICACION </th>
			   <th> STATUS </th></tr>";
