...
<?php
	...
	$bd = new DataBase();
$titulo  = " REPORTE ASISTENCIA DIARIA CONCEPTOS ";
$archivo = "reportes/rp_nom_as_concepto_det.php?Nmenu=$Nmenu&mod=$mod";
...
function Add_filtroX(){  // CARGAR  ARCHIVO DE AJAX CON UN PARAMETRO //
	...
	if(error == 0){
		...
		ajax=nuevoAjax();
			ajax.open("POST", "ajax_rp/Add_nom_as_concepto.php", true);
			...
			ajax.send("rol="+rol+"&region="+region+"&estado="+estado+"&concepto="+concepto+"&contrato="+contrato+"&trabajador="+trabajador+"&fecha_desde="+fecha_desde+"&fecha_hasta="+fecha_hasta+"");
...
<form name="form_reportes" id="form_reportes" action="<?php echo $archivo;?>"  method="post" target="_blank">
	<hr /><table width="100%" class="etiqueta">
        ...
        <td><?php echo $leng['concepto']?>: </td>
			<td><select name="concepto" id="concepto" style="width:120px;">
					<option value="TODOS">TODOS</option>
					<?php
	   			$query01 = $bd->consultar($sql_concepto_as);
		 		while($row01=$bd->obtener_fila($query01,0)){
					 echo '<option value="'.$row01[0].'">'.$row01[1].'</option>';
			   }?></select></td>
