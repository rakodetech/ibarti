...
$Nmenu = 536;
$archivo = "rp_asistencia_profit&id=".$id."&Nmenu=".$Nmenu."";
$tabla   = "asistencia_profit";
require_once('autentificacion/aut_verifica_menu.php');

?>
<br>
<div align="center" class="etiqueta_title"> Reporte Asistencia  <?php echo $leng['concepto']?> PROFIT <BR /></div>
<div id="Contendor01" class="mensaje"></div>
<br/>
<form id="reportes" action="reportes/rp_asistencia_profit_det.php?Nmenu=<?php echo $Nmenu?>" method="post" target="_blank">
  <table width="750px" border="0" align="center">
    ...
    <tr>
      ...
      <td id="date01" width="70%"><input type="text" name="fecha_desde" maxlength="120" style="width:100px" value="<?php echo $date?>"/>	          <img src="imagenes/ok.gif" alt="Valida" class="validMsg" border="0"/><br>
    ...
    <tr>
		...
		<td  id="select01">
			   <select name="region" style="width:250px;">
					   ...
					   <?php
					    $query04 = mysql_query("SELECT id, descripcion FROM regiones ORDER BY 2", $cnn);
					   while($row04=(mysql_fetch_array($query04))){
    ...
    <tr>
		...
		<td  id="select02">
			   <select name="contracto" style="width:250px;">
     				   <option value="TODOS"> TODOS</option>
					   <?php
					    $query03 = mysql_query(" SELECT nomina.co_cont, nomina.des_cont FROM nomina
						                          WHERE nomina.tip_cont = '2' ORDER BY 2 ASC ", $cnn);
    ...
    <tr>
        <td class="etiqueta"><?php echo $leng['nomina']?> Categoria:</td>
		<td  id="select02">
			   <select name="categoria" style="width:250px;">
     				   <option value="TODOS"> TODOS</option>
					   <?php
					    $query03 = mysql_query("SELECT snvaria_categoria.id, snvaria_categoria.descripcion
                                                  FROM snvaria_categoria
                                                 WHERE snvaria_categoria.`status` = 1 ORDER BY 2 ASC ", $cnn);
					   while($row03=(mysql_fetch_array($query03))){
					   echo '<option value="'.$row03[0].'">'.$row03[1].' ('.$row03[0].')</option>';
					   }
					   ?>
