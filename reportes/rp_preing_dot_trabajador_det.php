...
if($cargo != "TODOS"){
	...
	// QUERY A MOSTRAR //
        $sql = "SELECT v_preingreso.fec_us_mod, 
        			   IFNULL(roles.descripcion, 'SIN ASIGNAR') AS rol,v_preingreso.estado,
                  ...
                  LEFT JOIN trab_roles ON v_dot_sin_dotacion.cod_ficha = trab_roles.cod_ficha
LEFT JOIN roles ON trab_roles.cod_rol = roles.codigo
                $where
			  ORDER BY 1 DESC  ";
	...
	if($reporte== 'excel'){
	 ...
	 echo "<table border=1>";
 	 echo "<tr><th> Fecha Ult. Modificación </th><th> ".$leng['rol']." </th><th> ".$leng['estado']." </th><th> ".$leng['ciudad']." </th>
