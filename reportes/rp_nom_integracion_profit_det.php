  ...
  $sql = " CREATE TEMPORARY TABLE temp_hora_extra AS
    ...
    echo "</tbody>
		...
		if($reporte == 'txt'){
				...
				$sql = "CREATE TEMPORARY TABLE temp_hora_extra AS
				SELECT asistencia.cod_ficha, control.cod_hora_extras,
				       asistencia.hora_extra
				  FROM asistencia_apertura ,  asistencia, control
				 WHERE asistencia_apertura.fec_diaria  BETWEEN \"$fecha_D\" AND \"$fecha_H\"
				   AND asistencia_apertura.codigo = asistencia.cod_as_apertura
				   AND asistencia.hora_extra > 0
				...
				SELECT asistencia.cod_ficha, control.cod_hora_extras_n,
			...
			$sql01 = "SELECT  a.cod_hora_extras,  a.cod_ficha,  SUM(a.hora_extra) cantidad
								  FROM temp_hora_extra a, ficha , trab_roles
								  WHERE a.cod_ficha = ficha.cod_ficha
								  AND  a.cod_ficha = trab_roles.cod_ficha
								  $where
								 GROUP BY a.cod_ficha, a.cod_hora_extras
           		    ORDER BY 2 ASC ";
		Header("Content-Type: text/plain");
	//	Header("Content-Disposition: attachment; filename=../txt/asistencia.txt");
		Header("Content-Disposition: attachment; filename=".$archivo.".txt");
	   ...
	   $sql = "SELECT roles.descripcion rol,  contractos.descripcion contracto,
                    estados.descripcion estado, ciudades.descripcion ciudad,
                     ficha.cod_ficha, ficha.cedula, CONCAT(ficha.apellidos, ' ',ficha.nombres) trabajador,
