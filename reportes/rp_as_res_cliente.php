...
?>
<br>
<div align="center" class="etiqueta_title"> Reporte Resumen Asistencia Por <?php echo $leng['cliente']?><BR /><BR /></div>
<div id="Contendor01" class="mensaje"></div>
<br/>
<form id="clientes" name="clientes" action="reportes/rp_as_res_cliente_det.php?Nmenu=<?php echo $Nmenu?>" method="post" target="_blank">
    ...
    <tr>
		...
		<td  id="select01">
			   <select name="cliente" style="width:300px;">
     				   <option value="TODOS"> TODOS</option>
					   <?php
					    $query02 = mysql_query("SELECT co_cli, cli_des FROM clientes ORDER BY 2 ASC", $cnn);
					   while($row02=(mysql_fetch_array($query02))){
					   echo '<option value="'.$row02[0].'">'.$row02[1].' ('.$row03[0].')</option>';
					   }
					   ?>
	           </select><img src="imagenes/ok.gif" alt="Valida" class="validMsg" border="0"/><br />
