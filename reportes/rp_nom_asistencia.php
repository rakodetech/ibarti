...
<?php
	...
	$bd = new DataBase();
	$archivo = "reportes/rp_nom_asistencia_det.php?Nmenu=$Nmenu&mod=$mod";
	$titulo  = " REPORTE ASISTENCIA DIARIA ";
...
function Add_filtroX(){  // CARGAR ARCHIVO DE AJAX CON UN PARAMETRO //
	...
	if(error == 0){
		...
		ajax=nuevoAjax();
			ajax.open("POST", "ajax_rp/Add_nom_asistencia.php", true);
			...
			ajax.send("rol="+rol+"&region="+region+"&estado="+estado+"&ciudad="+ciudad+"&cliente="+cliente+"&ubicacion="+ubicacion+"&contrato="+contrato+"&trabajador="+trabajador+"&fecha_desde="+fecha_desde+"&fecha_hasta="+fecha_hasta+"");
...
<form name="form_reportes" id="form_reportes" action="<?php echo $archivo;?>"  method="post" target="_blank">
	<hr /><table width="100%" class="etiqueta">
      ...
      <td colspan="2"><input  id="stdName" type="text" style="width:190px" disabled="disabled" />
