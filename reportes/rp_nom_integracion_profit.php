...
$Nmenu   = 545;
$mod     =  $_GET['mod'];
$titulo  = " Reporte Integracion ".$leng['nomina']."";
$archivo = "reportes/rp_nom_integracion_profit_det.php?Nmenu=$Nmenu&mod=$mod";
...
function Add_filtroX(){  // CARGAR  ARCHIVO DE AJAX CON UN PARAMETRO //
	...
	if(error == 0){
		...
		document.getElementById("cont_img").innerHTML = '<img src="imagenes/loading.gif" onclick="" class="imgLink" />';

		ajax=nuevoAjax();
			ajax.open("POST", "ajax_rp/Add_nom_integracion_profit.php", true);
			...
			ajax.send("rol="+rol+"&categoria="+categoria+"&estado="+estado+"&ciudad="+ciudad+"&contrato="+contrato+"&trabajador="+trabajador+"&fecha_desde="+fecha_desde+"&fecha_hasta="+fecha_hasta+"");
	...
	<hr /><table width="100%" class="etiqueta">
        ...
        <td width="10%">Fecha Hasta:</td>
		 ...
		 <td width="10%">Categoria: </td>
			<td width="14%" id="select01"><select name="categoria" id="categoria" style="width:120px;" required>
		...
		<td id="select02"><select name="contrato" id="contrato" style="width:120px;" required>
      ...
      <td width="4%" id="cont_img"><img class="imgLink" src="imagenes/actualizar.png" border="0" onclick="Add_filtroX()"></td>
        ...
        <tr>
		 ...
		 <td><select name="rol" id="rol" style="width:120px;" required>
...
<div align="center"><br/>
		...
		<img class="imgLink" src="imagenes/word.gif" border="0" 
    	onclick="{$('#reporte').val('txt');$('#procesar').click();}" width="25px" title="imprimir a txt">
</div>	
</form>
