		...
		var errorMessage = ' ';
		  if(error == 0){
		    var parametros = {  };
		      $.ajax({
		          data:  parametros,
		          url:   'modulo/hora/views/Cons_inicio.php',
		          type:  'post',
		          success:  function (response) {
              $("#Cont_hora").html(response);
		          },
		...
		var errorMessage = ' ';
		  if(error == 0){
		    var parametros = { "codigo" : cod,
		      ...
		      $.ajax({
		          data:  parametros,
		          url:   'modulo/hora/views/Add_form.php',
		          type:  'post',
		          success:  function (response) {
              $("#Cont_hora").html(response);
		          },
...
function save_hora(){
	...
	if(error == 0){
		...
		var parametros = {"codigo": codigo, 	  			"activo": activo,
							...
							$.ajax({
									data:  parametros,
									url:   'modulo/hora/modelo/hora.php',
									type:  'post',
									success:  function (response) {
