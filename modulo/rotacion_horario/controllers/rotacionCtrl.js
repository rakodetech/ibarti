...
function Cons_rotacion_inicio(){
	...
	var errorMessage = ' ';
      ...
      $.ajax({
          data:  parametros,
          url:   'modulo/rotacion_horario/views/Cons_inicio.php',
          type:  'post',
...
function Cons_rotacion(cod, metodo){
	...
	var errorMessage = ' ';
      ...
      $.ajax({
          data:  parametros,
          url:   'modulo/rotacion_horario/views/Add_form.php',
          type:  'post',
          success:  function (response) {
          $("#Cont_rotacion").html(response);
						if(metodo == "modificar"){
...
function save_rotacion(){
	...
	if(error == 0){
		...
		var parametros = {"codigo": codigo, 	  			"activo": activo,
				...
				$.ajax({
						data:  parametros,
						url:   'modulo/rotacion_horario/modelo/rotacion_horario.php',
						type:  'post',
						success:  function (response) {
	...
	function CargarDetalle(cod){
			...
			$.ajax({
					data:  parametros,
					url:   'modulo/rotacion_horario/views/Add_form_det.php',
					type:  'post',
					success:  function (response) {
	...
	function Mostrar_Detalle(cod, cont_id){
		if(cod !=""){
			...
			$.ajax({
					data:  parametros,
					url:   'modulo/rotacion_horario/modelo/turno_det.php',
					type:  'post',
					success:  function (response) {
					var resp = JSON.parse(response);
					 if(resp.error){
	...
	function save_det(cod, metodo){
		...
		if(error == 0){
			var parametros = {"codigo": cod, 	  		"rotacion": rotacion,
					...
					$.ajax({
							data:  parametros,
							url:   'modulo/rotacion_horario/modelo/rotacion_horario_det.php',
							type:  'post',
							success:  function (response) {
								 var resp = JSON.parse(response);
								if(resp.error){
