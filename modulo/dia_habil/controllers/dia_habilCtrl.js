		...
		var errorMessage = ' ';
		  if(error == 0){
		    var parametros = {  };
		      $.ajax({
		          data:  parametros,
		          url:   'modulo/dia_habil/views/Cons_inicio.php',
		          type:  'post',
		...
		var errorMessage = ' ';
		  if(error == 0){
		    var parametros = { "codigo" : cod,
		      ...
		      $.ajax({
		          data:  parametros,
		          url:   'modulo/dia_habil/views/Add_form.php',
		          type:  'post',
		          success:  function (response) {
              $("#Cont_dia_habil").html(response);
								if(metodo = "modificar"){
...
function save_dia_habil(){
	...
	if(error == 0){
			var parametros = {"codigo": codigo, 	  		"activo": activo,
					...
					$.ajax({
							data:  parametros,
							url:   'modulo/dia_habil/modelo/dia_habil.php',
							type:  'post',
							success:  function (response) {
	...
	function Add_dh_det(cod){
		...
		var parametros = { "codigo" : codigo,   "cod_dia_tipo" : cod,
			...
			$.ajax({
					data:  parametros,
					url:   'modulo/dia_habil/views/Add_dia.php',
					type:  'post',
					success:  function (response) {
