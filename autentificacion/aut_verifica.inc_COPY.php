...
// Chequeamos si se está autentificandose un usuario por medio del formulario Login
...
*/
     ...
     $query = "SELECT $tb_usuario.codigo, $tb_usuario.cod_perfil, $tb_usuario.cedula, $tb_usuario.nombre, 
	                  $tb_usuario.apellido, $tb_usuario.login, $tb_usuario.pass, $tb_usuario.pass_old, $tb_usuario.email, 
					  DATEDIFF(NOW(), $tb_usuario.fec_mod_pass) AS dias_caduca,
                      $tb_usuario.status, clientes.abrev AS cl_abrev, clientes.nombre AS cl_nombre
		         FROM $tb_usuario, control, clientes 
...
//   pg_close($cnn);
  ...
  if ($login != $usuario_datos['login']){
	...
	$_SESSION["cl_abrev"]     = $usuario_datos['cl_abrev'];
