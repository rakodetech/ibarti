...
$result01 = mysql_query("SELECT cont_clientes.cod_cont_clasif, cont_clientes_clasif.descripcion AS clasif, 
                                cont_clientes.id_dpt1,  dpt_1.entidescri, 
								cont_clientes.id_dpt2,  dpt_2.munidescri, 
								cont_clientes.descripcion,
								cont_clientes.direccion, cont_clientes.telefono,
								cont_clientes.`status`
					       FROM cont_clientes , cont_clientes_clasif , dpt_1 , dpt_2
						  WHERE cont_clientes.codigo = '$codigo' 
							AND cont_clientes.cod_cont_clasif = cont_clientes_clasif.codigo
							AND cont_clientes.id_dpt1 = dpt_1.entienti 
							AND cont_clientes.id_dpt2 = dpt_2.id", $cnn)or die
...
<form action="sc_maestros/<?php echo $archivo;?>.php" method="post" name="Mod" id="Mod"> 
    ...
    <tr>
      ...
      <td id="input01" width="75%"><input type="text" name="codigo" maxlength="16" style="width:150px" 
    ...
    <tr>
      ...
      <td id="input02"><input type="text" name="descripcion" maxlength="60" value="<?php echo $row01['descripcion'] ?>" 
                              style="width:250px" />	  
        <img src="imagenes/ok.gif" alt="Valida" class="validMsg" border="0"/><br>
        <span class="textfieldRequiredMsg">La Descripcion es Requerida.</span>
		<span class="textfieldMinCharsMsg">Debe Escribir mínimo 2 Caracteres.</span></td>
    ...
    <tr>
     ...
     <tr>
	     ...
	     <td id="select01">		    
			   <select name="clasif" style="width:200px;">	 
     				   <option value="<?php echo $row01['cod_cont_clasif'] ?>"><?php echo $row01['clasif'] ?></option>
					   <?php 
					   $query03 = mysql_query("SELECT * FROM cont_clientes_clasif WHERE status =  1 ORDER BY 2 ASC ", $cnn);
     ...
     <tr>
	     ...
	     <td id="select02">		    
			   <select name="estado" style="width:200px;" onchange="Activar01(this.value)">	 
     				   <option value="<?php echo $row01['id_dpt1'] ?>"><?php echo $row01['entidescri'] ?></option>
					   <?php
     ...
     <tr>
	     <td height="20" class="etiqueta">Municipio:</td>
	     <td id="select03">
		 <?php 
		 $query03 = mysql_query("SELECT id, munidescri FROM dpt_2 WHERE relacion = ".$row01['id_dpt1']." ORDER BY munidescri", $cnn);

	echo'<select name="municipios" style="width:200px" onchange="Validar01(this.value)">
			     <option value="'.$row01['id_dpt2'].'">'.$row01['munidescri'].'</option>'; 
			  	 while($row03=mysql_fetch_array($query03))
				 {
		echo '<option value="'.$row03[0].'">'.$row03[1].'</option>';
				}
