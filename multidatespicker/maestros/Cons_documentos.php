	...
	<table width="90%" border="0" align="center">
		<tr class="fondo00">
			...
			if ($tabla == 'documentos' || $tabla == 'documentos_cl') {
				echo '<th width="15%" class="etiqueta">Orden</th>';
			}
			?>
		...
		$valor = 0;
		if ($tabla == 'documentos' || $tabla == 'documentos_cl') {
			$sql = " SELECT codigo, descripcion, status, orden FROM $tabla ORDER BY 2 ASC ";
		} else {
		...
		while ($datos = $bd->obtener_fila($query, 0)) {
			...
			if ($tabla == 'documentos' || $tabla == 'documentos_cl') {
				echo ' <td class="texto">' . $datos[3] . '</td> ';
			}
			echo '<td class="texto">' . statuscal($datos[2]) . '</td>
