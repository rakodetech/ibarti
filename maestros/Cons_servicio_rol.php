<?php 
	...
	$tabla = "servicio_rol";
	$bd = new DataBase();
	$archivo = "servicio_rol";
	$titulo = " SERVICIOS ROLES "; 
   ...
   $query = $bd->consultar($sql);
	...
	// $Modificar = "Add_Mod01('".$datos[0]."', 'modificar')";
	   $Borrar = "Borrar01('".$datos[0]."')";
        echo '<tr class="'.$fondo.'"> 
                  <td class="texto">'.utf8_decode($datos[0]).'</td> 
                  <td class="texto">'.utf8_decode($datos[1]).'</td>
				  <td class="texto">'.statuscal(utf8_decode($datos[2])).'</td>
