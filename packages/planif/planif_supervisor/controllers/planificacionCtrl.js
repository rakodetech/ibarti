...
function cargar_planif_superv(ap) {
	...
	if (ubic == '') {
		...
		if (ap != '') {
			apertura = ap;
			cargar_supervision_det(ubic);
			cargar_planif_superv_det(ubic);
		}
	}
}
...
function replicar_rot() {
	if (confirm("Esta seguro(a) de que desea replicar las rotaciones anteriores?..")) {
		ubic = $("#planf_ubicacion").val();
		var parametros = { "apertura": apertura, "supervision": supervision, "cliente": cliente, "ubicacion": ubic, "usuario": usuario };
		$.ajax({
			data: parametros,
			url: 'packages/planif/planif_supervisor/modelo/replicar_planif_rot.php',
			type: 'post',
			beforeSend: function () {
...
function cargar_supervision_det(ubic) {
	var parametros = {
		"ubicacion": ubic,
		"usuario": usuario
	};
...
function Cargar_puesto(codigo, contenedor) {
	...
	$.ajax({
		data: parametros,
		url: 'packages/cliente/cl_supervision/views/select_puesto.php',
		type: 'post',
		success: function (response) {
...
function generar_planif(ubic) {
	if ((ubic != "") && (apertura != "")) {
		...
		$.ajax({
			data: parametros,
			url: 'packages/planif/planif_supervisor/modelo/generar_planif.php',
			type: 'post',
			success: function (response) {
...
function cargar_planif_superv_det(ubic) {
	var parametros = { "codigo": apertura, "ubicacion": ubic, "usuario": usuario };
	$.ajax({
		data: parametros,
...
function cargar_rotacion_posicion(rotacion, contenedor) {
	...
	$.ajax({
		data: parametros,
		url: 'packages/planif/planif_supervisor/views/Add_rotacion_posicion.php',
		type: 'post',
		beforeSend: function () {
...
function save_planif_det(cod) {
	...
	if (error == 0) {
		...
		$.ajax({
			data: parametros,
			url: 'packages/planif/planif_supervisor/modelo/planificacion.php',
			type: 'post',
			success: function (response) {
				cargar_planif_superv_det(ubic);
				var resp = JSON.parse(response);
...
function save_planif_trab_det(cod, metodo) {
	if (confirm("�Esta Seguro De Actualizar Este Registro")) {
		...
		$.ajax({
			data: parametros,
			url: 'packages/planif/planif_supervisor/modelo/planificacion_trab_det.php',
			type: 'post',
			success: function (response) {
				...
				if (resp.error) {
					...
					cargar_planif_superv_trab_det(planif_cl_trab, 'SI');
					alert('Actualizacion Exitosa!..')
				}
			},
			error: function (xhr, ajaxOptions, thrownError) {
...
function Calendario(cod) {
	...
	if (error == 0) {
		...
		$.ajax({
			data: parametros,
			url: 'packages/planif/planif_supervisor/views/Add_calendario.php',
			type: 'post',
			success: function (response) {
				ModalOpen();
...
function Borrar_trab_det(cod) {
	...
	if (confirm("�Esta Seguro De Borrar el Registro(" + ficha + ") de esta Planificación?")) {
		...
		$.ajax({
			data: parametros,
			url: 'packages/planif/planif_supervisor/modelo/delete_planif_trab_det.php',
			type: 'post',
			success: function (response) {
				var resp = response;
				if (resp.error) {
...
function B_reporte(detalle) {
	...
	var parametros = {
		...
		"supervision": supervision, "usuario": usuario
	};
	if (errorMessage == '') {
		$.ajax({
			data: parametros,
			url: 'ajax/Add_planif_servicio_superv_min.php',
			type: 'post',
			beforeSend: function () {
				$("#modal_contenidoRP").html('');
				$('#modalRP').show();
				$("#RP").html('<img src="imagenes/loading3.gif" border="null" class="imgLink" width="30px" height="30px"> Procesando...');
				$("#cod_supervision_serv").val($("#planif_supervision").val());
			},
			success: function (response) {
				var resp = JSON.parse(response);
			...
			elementos_cantidad.append("img").attr("id", (d) => `mod_${d.codigo}_${d.fecha}_${d.cod_cliente}_${d.cod_ubicacion}_${d.cod_turno}`).attr("src", "imagenes/ico_guadar.ico").style("width", "20px").style("margin-left", "10px").style("display", "none").attr("title", "modificar registro").on("click", (d) => {
				if (confirm("Esta seguro que desea modificar")) {
					...
					ac_apertura_planif(propiedades, () => {
						$(`#mod_${d.codigo}_${d.fecha}_${d.cod_cliente}__${d.cod_ubicacion}_${d.cod_turno}`).hide();
