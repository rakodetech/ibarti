...
function agregar_arreglo_contactos() {
	...
	var error = 0;
	var motivo_error="";
	$('#data_ad *').filter(':input').each(function (pos, elemento) {
		if(elemento.value==""){
			
			if(!(elemento.id=="c_observacion")){
				error+=1;
				motivo_error+= `Tiene que llenar el campo: ${elemento.id.replace('c_','')}\n`;
			}
			
		}
		if (pos == 0) {
			datos += `"${elemento.id.replace('c_', '')}":"${elemento.value.toUpperCase()}"`;
		} else {
			datos += `,"${elemento.id.replace('c_', '')}":"${elemento.value.toUpperCase()}"`;
		}
	...
	switch (tipo) {
		...
		case 'modificar':
			...
			$('#data_ad *').filter(':input').each(function (pos, elemento) {
				...
				document.getElementById(elemento.id).value = arreglo_contactos[pos_modificar][elemento.id.replace('c_', '')];
