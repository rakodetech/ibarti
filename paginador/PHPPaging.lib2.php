...
class PHPPaging {
	...
	**/
	...
	var $mostrarPrimera = '<img src="imagenes/navigate_left2.ico" alt="Primera" />';
	/**
	...
	**/
	...
	var $mostrarUltima = '<img src="imagenes/navigate_right2.ico" alt="Ultima" />';
	/**
	...
	**/
	//var $mostrarAnterior = "&lt; Anterior";
	var $mostrarAnterior = '<img src="imagenes/navigate_left.ico" alt="Anterior" />';

	var $VolverAtras = '<img src="imagenes/undo.ico"  onclick="history.back(-1);" title="Volver" class="imgLink"/>';
	
	var $navegarcion =  ' <b>Navegación</b>:';	
//	
	/**
	...
	**/   
//	var $mostrarSiguiente = 'Siguiente &gt;';
	var $mostrarSiguiente = '<img src="imagenes/navigate_right.ico" alt="Siguiente" />';
...
*/
	...
	**/
	function fetchNavegacion () {
		...
		$link_string .= $this->navegarcion;    
		$link_string .= $this->linkSeparador.$this->VolverAtras; 
		return $link_string;
	}
	
	function VolverLink () {
		$Volver = $this->linkSeparador.$this->VolverAtras;
		return $Volver;
	}	
	...
	*/
	function ejecutar () {
		...
		if(!empty($this->sql)) {
			$result = (isset($this->conn)) ? @pg_query($this->conn, $this->sql) : @pg_query($this->sql);
			if(!$result) return $this->error(false, " @mysql_error(), $this->sql"); // version 2.0
			$this->numTotalRegistros = @pg_num_rows($result);
		} else {
		...
		if($this->numTotalRegistros >= 1) {
			...
			if(!empty($this->sql)) {
				...
				$result = (isset($this->conn)) ? @pg_query($this->conn, $sql) : @pg_query($sql);
				if(!$result) return $this->error(false, "aaa");
				//if(!$result) return $this->error(false, @mysql_error(), $sql, ereg("limit[ ]+[0-9]+(,[ ]*[0-9]+)?", strtolower($sql))); // version 2.0
				$this->ejecutard = $result;
				$this->numTotalRegistros_actual = pg_num_rows($result);
			} else {
