...
$usuario   = $_POST['usuario'];
	...
	$where = " WHERE v_ficha.cod_ficha_status = control.ficha_activo
                 AND v_ficha.cod_ficha = ficha_dl.cod_ficha
...
$sql = " SELECT DISTINCT v_ficha.cod_ficha, v_ficha.cedula,
                v_ficha.ap_nombre, ficha_dl.cod_cliente,
                clientes.nombre AS cliente, ficha_dl.cod_ubicacion,
                clientes_ubicacion.descripcion AS ubicacion
           FROM v_ficha, control, ficha_dl, clientes, clientes_ubicacion
         $where
       ORDER BY 3 ASC ";   
...
<table width="100%" border="0" align="center">
   ...
   $query = $bd->consultar($sql);
		...
		$valor = 1;
			...
			$cod_cliente_x = $datos[3];
			$cliente_x     = $datos[4];
			$cod_ubicacion = $datos[5];
			$ubicacion     = $datos[6];

/*
<input name="ficha[]"  type="checkbox" value="'.$ficha.'" style="width:auto"
				                           checked="checked" />&nbsp;'.$ficha.'</td> */ 
	   $aj = "Add_cliente(this.value, '".$ficha."', 'ajax/Add_pl_ubicacion.php', 'ubic_".$ficha."')";
        echo '<tr class="'.$fondo.'"> 
				  <td class="texto"><input type="hidden" name="ficha[]" value="'.$ficha.'"/>&nbsp;'.$ficha.'</td>
	...
	$sql_cliente = "SELECT clientes_ubicacion.cod_cliente, clientes.nombre AS cliente
			...
			echo'</select></td>
				  <td class="texto" id="ubic_'.$ficha.'"><select name="ubicacion_'.$ficha.'" style="width:200px;"> 
		...
		$sql_ubicacion = "SELECT clientes_ubicacion.codigo, clientes_ubicacion.descripcion
			...
			while($row03=$bd->obtener_fila($query03,0)){
				...
				echo '<td class="texto"><input name="trabajador[]" type="checkbox" 
		                  value="'.$row03[0].'-'.$ficha.'" style="width:auto" '.$checkX.' disabled="disabled" /></td>';
			}							 
