...
$where = " WHERE nov_check_list_planif_actividad.cod_nov_clasif = nov_clasif.codigo
				AND nov_check_list_planif_actividad.cod_nov_tipo   = nov_tipo.codigo
				AND nov_clasif.codigo = nov_perfiles.cod_nov_clasif
                AND nov_perfiles.cod_perfil = '$perfil'
                AND nov_check_list_planif_actividad.cod_cliente = clientes.codigo
                AND nov_check_list_planif_actividad.cod_ubicacion = clientes_ubicacion.codigo
                AND nov_check_list_planif_actividad.cod_ficha = ficha.cod_ficha
                AND nov_check_list_planif_actividad.cod_nov_status = nov_status.codigo
				AND nov_check_list_planif_actividad.cod_actividad = planif_actividad.codigo
				AND planif_actividad.cod_proyecto = planif_proyecto.codigo";

if ($codigo != "" && $codigo != null) {
	$where .= " AND nov_check_list_planif_actividad.codigo = '$codigo' ";
}

if ($fecha_D != '0000-00-00' && $fecha_H != '0000-00-00') {
	$where .= " AND nov_check_list_planif_actividad.fec_us_ing BETWEEN \"$fecha_D\" AND \"$fecha_H\"";
}
if ($clasif != "TODOS") {
...
if ($cod_ficha != "" && $cod_ficha != null) {
	$where .= " AND nov_check_list_planif_actividad.cod_ficha = '$cod_ficha' ";
}

$sql   = " SELECT nov_check_list_planif_actividad.codigo, nov_check_list_planif_actividad.fec_us_ing,
					  ...
					  planif_proyecto.descripcion proyecto,
						planif_actividad.descripcion actividad,nov_status.descripcion AS `status`
                 FROM nov_check_list_planif_actividad , nov_clasif , clientes , clientes_ubicacion ,
                      ficha , nov_status , nov_perfiles , nov_tipo, planif_proyecto, planif_actividad
               $where
                  ORDER BY 2 DESC ";
...
?><table width="100%" align="center">
	...
	<tr class="fondo00">
		...
		<th width="18%" class="etiqueta">Proyecto</th>
		<th width="18%" class="etiqueta">Actividad</th>
	...
	while ($datos = $bd->obtener_fila($query, 0)) {
		...
		echo '<tr class="' . $fondo . '">
				  ...
				  <td class="texo">' . longitudMin($datos["proyecto"]) . '</td>
				  <td class="texo">' . longitudMin($datos["actividad"]) . '</td>
